...
def func(get_or_post, enable_session, rep,
         ...
         logged:bool = True,global_parameters:dict = None):
    '''
    ...
    if (logged):
        ...
        if other('CoreConfiguration',session=session).UserLoginAuthentication(session):
            ...
            return rep(json.dumps(instructs_run(instruction_set,instruction_name_list,instruction_func_list,dependency_table,global_parameters)))
        else:
            return rep(json.dumps(['You are not logged in']))
    else:
        instruction_set = json.loads(get_or_post('instruction_set'))
        return rep(json.dumps(instructs_run(
            instruction_set= instruction_set,
            instruction_name_list= instruction_name_list,
            instruction_func_list= instruction_func_list,
