...
class Lci_user {
    ...
    OtherKey = '';
    SessionKey = null;
    reqapi(name,parameter,Call=function(data){console.log(data)}){
        // 请求API
        // name :API名字
        // parameter :参数
    ...
    signup(name,Key,nickname,postbox=None,Call=function(data){console.log(data)}){
        `
        注册:
        name 用户名
        key 密码(几乎没用)
    ...
    logon(id=null,name=null,postbox=null,Call=function(data){console.log(data)}){
        // 登录
    ...
    logonValidate(ValidateCode=null,Call=function(data){console.log(data)}){
        // 登录，验证验证码是否正确
    ...
    LoginStatus(Call=function(data){console.log(data)}){
        // 获取登录状态
        return this.reqapi('login_status',{
        },Call
        ) 
    }
}